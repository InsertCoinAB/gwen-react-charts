...
export const Sankey: React.FC<Props> = (props: Props) => {
  ...
  const stepsTotal: Record<number, number> = props.data.reduce(
    (prev, curr) => ({ ...prev, [curr.step]: 0 }),
    {}
  );
  const prevStepsTotal: Record<number, number> =
    props.prevData?.reduce((prev, curr) => ({ ...prev, [curr.step]: 0 }), {}) ??
    props.data.reduce((prev, curr) => ({ ...prev, [curr.step]: 0 }), {});
  const maxSteps = Object.keys(stepsTotal).length;
  const endBehaviorId =
    props.data.find((i) => i.step === maxSteps)?.behaviorId ?? "";
  const mergedData = mergeNodes(props.data, endBehaviorId);
  const prevData = props.prevData
    ? mergeNodes(props.prevData, endBehaviorId)
    : mergedData;
  // Corrects first step total
  mergedData
    .filter((n) => n.step === 1)
    .forEach((n) => (stepsTotal[1] += n.amount ?? 0));
  ...
  data.nodes.forEach((n) => {
    ...
    if (
      !sources.includes(n.id) &&
      !n.id.includes("Drop Off") &&
      getCleanStep(n.id) !== maxSteps
    ) {
  ...
  data.links.forEach((link) => {
    ...
    const l = data.links.find(
      (l) => l.target === target && l.source === link.source
    );
    if (l) return;
    const valueFrom =
        ...
        : data.links
            .filter(
              (l) =>
                ...
                getCleanStep(l.source) === getCleanStep(link.source) - 1
            )
            .reduce((t, v) => t + v.value, 0);
    const valueTo = data.links
      .filter(
        (l) =>
          l.source === link.source &&
          getCleanStep(l.source) === getCleanStep(link.source)
      )
      .reduce((t, v) => t + v.value, 0);
  ...
  const prevValuesRecord: Record<string, number> = toPrevRecordData(
    prevData,
    prevStepsTotal,
    endBehaviorId,
    maxSteps,
  ...
  return (
    <div style={wrapperStyle} ref={ref}>
      <ResponsiveSankey
            ...
            <>
              ...
              >
                {step === 1
                  ...
                  : `${getValuePrefix(Math.round(node.value))}, ${getPercentage(
                      node.value,
                      stepsTotal[getCleanStep(node.id)]
                    )}, `}
                ...
                {getCleanId(node.id) !== "Other" &&
                  getValueChangeElement(
                    ...
                    step === 1
                      ? stepsTotal[getCleanStep(node.id)]
                      : prevStepsTotal[getCleanStep(node.id)] ?? 0,
                    colors
                  )}
              </tspan>
            </>
        ...
        nodeTooltip={({ node }) => (
          <div style={toolTipStyle()}>
            <b>{`${titleRecord[node.id] ?? getCleanId(node.id)} - ${getStepName(
              node.id,
              maxSteps
            )}`}</b>
            <br />
...
const getValueChangeElement = (
  ...
  if (prevValue > 0) {
    ...
    return (
      <tspan
        fill={change < 0 ? colors.labelDiffNeg : colors.labelDiffPos}
        textAnchor="start"
      >
        {change < 1 ? "" : "+"}
        {change !== 0 && `${change}%`}
...
const mergeNodes = (data: Node[], endBehaviorId: string) => {
  ...
  const lastStep = Object.keys(
    data.reduce((t, v) => ({ ...t, [v.step]: "" }), {})
  ).length;
  ...
  for (let i = 2; i <= lastStep; i++) {
    ...
    topBehaviors[i] = allStepNodes
      .splice(0, NODE_LIMIT + nodeRecuction)
      .map((n) => n.behaviorId);

    !topBehaviors[i].includes(endBehaviorId) &&
      topBehaviors[i].push(endBehaviorId);
  }
  ...
  sorted.forEach((n) => {
    ...
    if (n.step !== 1) {
      if (
        n.step !== lastStep &&
        !topBehaviors[n.step].includes(n.behaviorId) &&
        stepNodesTotal[n.step] > NODES_PER_STEP
      ) {
        behaviorId = "Other";
      }
      if (
        n.step !== 2 &&
        !topBehaviors[n.step - 1].includes(n.prevBehaviorId ?? "") &&
        stepNodesTotal[n.step - 1] > NODES_PER_STEP
      ) {
...
const toSankeyData = (
  ...
  colors: BehaviorFlowChartColors,
  stepsTotal?: Record<number, number>
) => {
  ...
  data
    ...
    .forEach((link) => {
      if (stepsTotal) {
        stepsTotal[link.step] =
          (stepsTotal[link.step] ?? 0) + (link.amount ?? 0);
      }
    });
...
const toPrevRecordData = (
  ...
  const links = data
    ...
    .map((link) => {
      const value = link.amount ?? 0;
      record[`${link.behaviorId}:${link.step}`] =
        (record[`${link.behaviorId}:${link.step}`] ?? 0) + value;
      return {
        source: `${link.prevBehaviorId}:${link.step - 1}`,
        target: `${link.behaviorId}:${link.step}`,
        value: value,
