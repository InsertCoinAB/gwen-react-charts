...
const BehaviorFlowChart: React.FC<BehaviorFlowChartProps> = ({
	...
	useEffect(() => {
		...
		const nodesList = behaviorFlow.behaviorFlowNodes.map((i) => ({ ...i, title: behaviorRecord ? behaviorRecord[i.behaviorId] ?? i.id : i.id }))
	...
	const uniqueSteps = [...new Set(nodes?.map((i) => i.step))].sort((a, b) => a - b)
	const steps = uniqueSteps.map((s, index) => ({ step: s, title: index === 0 ? "Start" : index === uniqueSteps.length - 1 ? "End" : `Step ${s - 1}` }))

	const c = { ...defaultColors, ...colors }

	return (
		<div style={{ display: "flex", flexDirection: "column", background: c.background, paddingTop: "20px" }} className={className}>
			{header && <div style={{ display: "flex", gap: "20px", marginBottom: "20px" }}>{<BehaviorSteps steps={steps} colors={c} />}</div>}
			<Sankey data={nodes.sort((a, b) => (b.amount ?? 0) - (a.amount ?? 0))} prevData={prevNodes} colors={c} />
		</div>
	)
}
...
export type GetMetricBehaviorFlowQuery = {
	...
	behaviors: { __typename?: "PaginatedBehaviorResponse"; items: Array<{ __typename?: "Behavior"; id: string; title: string }> }
	prevBehaviorFlow: {
		__typename?: "BehaviorFlow"
		id: string
		productId: string
